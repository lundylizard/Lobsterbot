    ...
    public void onMessageReceived(@Nonnull MessageReceivedEvent event) {
        ...
        if (message.getContentRaw().startsWith(Lobsterbot.PREFIX) && !message.getAuthor().getId().equals(event.getJDA().getSelfUser().getId())) {
            ...
            if (!Lobsterbot.blacklistManager.userBlacklisted(event.getMember())) {
                ...
                message.getAuthor().openPrivateChannel().queue(channel -> channel.sendMessage("You are blacklisted from using Lobster Bot.").queue());
        ...
        if (!message.getContentRaw().startsWith(Lobsterbot.PREFIX) && !message.getAuthor().isBot()) {
            ...
            if (!Lobsterbot.database.isInEXPDB(event.getMember())) {
                ...
                if (!(spam >= 2)) {
                    ...
                    Lobsterbot.experienceManager.addEXP(event.getMember(), (exp / 10));
                    Lobsterbot.experienceManager.addOverallEXP(event.getMember(), (exp / 10));
                    System.out.println("> " + event.getAuthor().getName() + " gained " + (exp / 10) + " EXP (EXP needed for next level: " + (Lobsterbot.experienceManager.calculateEXPneeded(Lobsterbot.experienceManager.getLevel(event.getMember())) - Lobsterbot.experienceManager.getEXP(event.getMember())) + ")");
